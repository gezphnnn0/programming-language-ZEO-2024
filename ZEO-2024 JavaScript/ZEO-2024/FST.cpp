...
void FST::LexAnalyzer(In::IN in, Out::OUT out, Log::LOG log, LT::LexTable& lextable, IT::IdTable& idtable)
{
	...
	libs[0].name = "compare";
	libs[0].parcount = 2;
	libs[0].params[0] = IT::STR;
	libs[0].params[1] = IT::STR;
	libs[0].retval = IT::BYTE;

	libs[1].name = "absolute";
	libs[1].parcount = 1;
	libs[1].params[0] = IT::BYTE;
	libs[1].retval = IT::BYTE;
...
#pragma region "FSTS"
	...
	FST l_write(
		...
		NODE(1, RELATION('i', 3)),
		NODE(1, RELATION('t', 4)),
		NODE(2, RELATION('e', 5), RELATION('e', 9)),
		NODE(1, RELATION('l', 6)),
		NODE(1, RELATION('i', 7)),
		NODE(1, RELATION('n', 8)),
		NODE(1, RELATION('e', 9)),
	...
	Checker checkArr[] = {
		Checker(&l_byte,LEX_UBYTE,IT::BYTE),
	...
	for (int i = 0; i < sizze; i++)
	{
		...
		for (int j = 0; j < checkArrLen; j++)
		{
			if (execute(*checkArr[j].chain))
			{
				...
				case LEX_TYPE: {
					if (checkArr[j].iddatatype == IT::BYTE)
					{
						dataType = IT::BYTE;
					}
				...
				case LEX_WRITE: {
					if (strlen(str) == 5) {
						lexTableObject.data = 'w';
					}
					else {
						lexTableObject.data = 'l';
					}
					break;
				}
				case LEX_ID: {
					...
					if (IT::IsId(idtable, (char*)scope.c_str()) == TI_NULLIDX)
					{
						...
						if (type != (IT::IDTYPE::P) && !isDeclare && !isExported) {
							throw ERROR_THROW_IN(713, lex.line, lex.col)
						}
